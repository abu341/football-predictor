import { EXCLUDED_LEAGUE_IDS } from "./leagues";
import {
  getFixturesByDate,
  getTeamForm,
  getH2H,
...
// h2h, injuries, full odds — the bulk /odds date fetch below only carries
// the Match Winner market, used for discovery/ranking, so the fixtures that
// make the final cut get their own full-odds call for O/U and BTTS). On top
// of that there's a flat per-day cost (one /fixtures call plus however many
// /odds pages that date has, typically ~55-60) shared regardless of this
// cap. Comfortable headroom on the daily quota (Pro plan, 7,500
// requests/day) — the real constraint is the Pro plan's 5 requests/second
// cap (see the throttle in lib/apiFootball.js), which is why this now runs
// from the daily cron job (app/api/archive/route.js) rather than inline on
// a page request: ~180 requests at a throttled pace takes on the order of a
// minute, too slow for a normal page render.
...
export async function buildPredictionsForDate(date) {
  ...
  // Every fixture worldwide for the date, and every fixture's odds for the
  // date, each in their own (small) fixed number of requests regardless of
  // how many leagues are actually playing — no more looping per league.
  const [allFixtures, oddsEntries] = await Promise.all([
    getFixturesByDate(date),
    getOddsForDate(date),
  ]);

  const oddsByFixtureId = new Map(oddsEntries.map((o) => [o.fixture.id, o]));

  // Odds availability is the real filter: bookmakers don't price friendlies,
  // so requiring odds keeps only fixtures worth predicting without
  // hand-picking leagues — but it doesn't reliably exclude youth/reserve
  // leagues on its own (confirmed live: some bookmaker prices even those),
  // hence EXCLUDED_LEAGUE_IDS as a manual backstop. Also drops anything not
  // kicking off as scheduled (already finished/live/postponed/cancelled) —
  // nothing to predict on a match that's already underway.
  const eligibleFixtures = (allFixtures || []).filter(
    (f) =>
      f.fixture.status?.short === "NS" &&
      oddsByFixtureId.has(f.fixture.id) &&
      !EXCLUDED_LEAGUE_IDS.includes(f.league.id)
  );

  // Rank by how many bookmakers price the match rather than by kickoff
  // time: confirmed live that "earliest kickoff first" can fill the whole
  // cap with whichever timezone's morning it happens to be (e.g. Australian
  // regional leagues dominating before Europe/Americas even kick off that
  // UTC date), starving out mainstream leagues elsewhere in the world that
  // just haven't kicked off yet. Bookmaker count is a reasonable proxy for
  // how mainstream a match is — a fixture 12 bookmakers price is far more
  // likely to be a real, well-covered match than one only 1-2 price. Sort
  // is stable, so same-count fixtures keep their original kickoff-time
  // order as a natural tiebreak.
  const byBookmakerCount = (f) => oddsByFixtureId.get(f.fixture.id).bookmakers.length;
  const fixturesToProcess = eligibleFixtures
    .slice()
    .sort((a, b) => byBookmakerCount(b) - byBookmakerCount(a))
  ...
  for (const fixture of fixturesToProcess) {
    ...
    try {
      const [homeFormRaw, awayFormRaw, h2hRaw, injuriesRaw, oddsRaw] = await Promise.all([
        ...
        // Full odds (every market) for just this fixture — the bulk fetch
        // above only carries Match Winner, used for discovery/ranking.
      ...
      // extractMatchWinnerOdds already returns null for an empty/missing
      // bookmakers array, covering the rare case where this fixture's full
      // odds come back empty despite being priced in the bulk discovery
      // fetch (which only checked the Match Winner market).
      const odds = extractMatchWinnerOdds(oddsRaw);
      ...
      const overUnderOdds = extractOverUnderOdds(oddsRaw, "2.5");
      const bttsOdds = extractBttsOdds(oddsRaw);
