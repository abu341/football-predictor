...
// Temporary diagnostic: for each tracked league, checks whether today's
// fixtures call succeeds and how many games it finds — used to figure out
// whether a league ID is wrong (shows an error) vs. that league just not
// playing today (shows 0 with no error). Safe to remove later; it's not
// used by the main prediction-building flow above.
export async function debugLeagueCoverage(date) {
  const rows = [];
  for (const league of TRACKED_LEAGUES) {
    try {
      const fixtures = await getFixturesByDateAndLeague(date, league.id);
      rows.push({
        id: league.id,
        label: leagueLabel(league.id),
